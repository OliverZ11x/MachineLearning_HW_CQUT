...
train_dataset = datasets.MNIST(root='./data/mnist', train=True,
                               transform=transform, download=True)  # 本地没有就加上download=True
test_dataset = datasets.MNIST(
    root='./data/mnist', train=False, transform=transform)  # train=True训练集，=False测试集
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv3 = torch.nn.Sequential(
            torch.nn.Conv2d(20, 40, kernel_size=3),
            torch.nn.ReLU(),
#            torch.nn.MaxPool2d(kernel_size=2),
        )
        self.fc = torch.nn.Sequential(
            torch.nn.Linear(360, 180),
            torch.nn.Linear(180, 90),
            torch.nn.Linear(90, 10),
        )

    def forward(self, x):
        batch_size = x.size(0)  # 取x张量中的第0个维度
